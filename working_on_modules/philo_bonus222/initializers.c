...
/*   Updated: 2024/02/09 07:59:01 by achappui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philosophers.h"

void	init_inputs(t_processdata *datas, int argc, char *argv[])
{
	int	i;
	...
	if (argc == 6)
	{
		...
		if (datas->meal_limit == 0)
			exit(write(1, "ERROR: 1 <= meal_limit \n", 24));
	}
	else
...
void init_semaphores(t_processdata *datas)
{
    ...
    datas->security = \
	sem_open("security", O_CREAT | O_EXCL, 0777, 1);
